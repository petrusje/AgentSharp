...
namespace HNSW.Net
{
        ...
        where TDistance : IComparable<TDistance>
    {
        ...
        /// <summary>
        /// Gets the number of items currently in the graph.
        /// </summary>
        public int Count => this.graph?.ItemCount ?? 0;
        ...
        /// <summary>
        /// Adds a single item to the existing HNSW graph incrementally.
        /// </summary>
        /// <param name="item">The item to add to the graph.</param>
        /// <param name="generator">Random number generator for level assignment.</param>
        /// <returns>The ID assigned to the added item.</returns>
        public int AddItem(TItem item, Random generator)
        {
            if (this.graph == null)
            {
                // Initialize graph with default parameters if it doesn't exist
                var parameters = new Parameters();
                this.graph = new Graph(this.distance, parameters);
                return this.graph.InsertNode(item, generator);
            }
        ...
        /// <summary>
        /// Adds a single item to the existing HNSW graph incrementally.
        /// Uses internal random generator for level assignment.
        /// </summary>
        /// <param name="item">The item to add to the graph.</param>
        /// <returns>The ID assigned to the added item.</returns>
        public int AddItem(TItem item)
        {
            return this.AddItem(item, new Random());
        }
