...
namespace HNSW.Net
{
    ...
    public partial class SmallWorld<TItem, TDistance>
    {
        ...
        internal class Graph
        {
            ...
            /// </summary>
            public int ItemCount => this.nextNodeId;

            /// <summary>
            /// Inserts a single item into the existing graph.
            ...
            /// <returns>The ID assigned to the inserted item.</returns>
            public int InsertNode(TItem item, Random generator)
            {
                var id = this.nextNodeId++;
                var newNode = this.NewNode(id, item, RandomLevel(generator, this.Parameters.LevelLambda));
                ...
                // Find the best peer on the same level as newNode
                ...
                // Connect new node to the small world
            ...
            public void Create(IList<TItem> items, Random generator)
            {
                ...
                // Reset node ID counter for batch creation
                this.nextNodeId = 0;
                ...
                for (id = 1; id < items.Count; ++id)
                {
                    ...
                    this.nextNodeId = id + 1; // Track the next ID
                    for (int level = bestPeer.MaxLevel; level > newNode.MaxLevel; --level)
                    {
                        bestPeer = KNearestAtLevel(bestPeer, newNode, 1, level).Single();
                    }
