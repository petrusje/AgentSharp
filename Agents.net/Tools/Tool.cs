...
namespace Agents.net.Tools
{
    ...
    public class Tool
    {
        ...
        private object ConvertValue(JsonElement value, Type targetType)
        {
            try
            {
                // Handle null values
                ...
                // For string type, handle both string and number values
                ...
                {
                    if (value.ValueKind == JsonValueKind.String && int.TryParse(value.GetString(), out int intVal))
                        return intVal;
                ...
                {
                    if (value.ValueKind == JsonValueKind.String && double.TryParse(value.GetString(), out double doubleVal))
                        return doubleVal;
                ...
                {
                    if (value.ValueKind == JsonValueKind.String && float.TryParse(value.GetString(), out float floatVal))
                        return floatVal;
                ...
                {
                    if (value.ValueKind == JsonValueKind.String && decimal.TryParse(value.GetString(), out decimal decimalVal))
                        return decimalVal;
                ...
                if (targetType == typeof(bool))
                {
                    ...
                    {
                        var strVal = value.GetString().ToLowerInvariant();
                ...
                {
                    if (value.ValueKind == JsonValueKind.String && DateTime.TryParse(value.GetString(), out DateTime dateVal))
                        return dateVal;
                    return value.GetDateTime();
                }

                throw new InvalidCastException($"Cannot convert JSON value of kind {value.ValueKind} to type {targetType.Name}");
            }
            catch (Exception ex)
            {
                Logger.Error($"Error converting JSON value to {targetType.Name}: {ex.Message}");
                throw new InvalidCastException($"Cannot convert JSON value to type {targetType.Name}: {ex.Message}", ex);
            }
        }
