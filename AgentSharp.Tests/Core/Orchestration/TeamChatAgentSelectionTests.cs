...
namespace AgentSharp.Tests.Core.Orchestration
{
    ...
    public class TeamChatAgentSelectionTests
    {
        ...
        private class MockAgent : IAgent
        {
            ...
            public async Task<object> ExecuteAsync(string input, object context = null, List<AIMessage> previousMessages = null, CancellationToken cancellationToken = default)
            {
                await Task.Delay(10, cancellationToken); // Simular processamento
                return $"Response from {_name}: {input}";
            }
        }
